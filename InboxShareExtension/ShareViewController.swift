...
final class ShareViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        statusLabel.text = "Wird verstanden …"
    ...
    private func importSharedContent() async {
        do {
            ...
            if FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: SharedInboxStore.appGroupID) != nil {
                var attachmentPath: String?
                if let data = captured.attachmentData {
                    attachmentPath = try SharedInboxStore.saveAttachment(
                        data: data,
                        fileExtension: captured.attachmentExtension ?? "bin"
                    )
                }

                let item = ContentAnalyzer.analyze(
                    text: captured.text,
                    sourceType: captured.sourceType,
                    sourceURL: captured.sourceURL,
                    attachmentRelativePath: attachmentPath
                )
                try SharedInboxStore.addItem(item)
                await finish("\(item.title) · gespeichert")
            } else {
                try writeClipboardFallback(captured)
                await finish("Erkannt · Inbox öffnen")
            }
        } catch {
    ...
    private func writeClipboardFallback(_ captured: CapturedContent) throws {
    ...
    private func captureFirstSupportedItem() async throws -> CapturedContent {
        ...
        for provider in providers {
            if provider.hasItemConformingToTypeIdentifier(UTType.pdf.identifier),
               let result = try await capturePDF(provider) { return result }

            if provider.canLoadObject(ofClass: UIImage.self),
               let result = try await captureImage(provider) { return result }
            ...
            if provider.hasItemConformingToTypeIdentifier(UTType.fileURL.identifier),
               let result = try await captureGenericFile(provider) { return result }
        }
        throw ShareError.noSupportedContent
    }
    ...
    private func captureImage(_ provider: NSItemProvider) async throws -> CapturedContent? {
        ...
        return CapturedContent(
            ...
            sourceType: .image,
            sourceURL: nil,
            attachmentData: image.jpegData(compressionQuality: 0.90),
    ...
    private func capturePDF(_ provider: NSItemProvider) async throws -> CapturedContent? {
        ...
        let data: Data?
        if let url = (value as? URL) ?? (value as? NSURL).map({ $0 as URL }) {
            data = try? Data(contentsOf: url)
        } else if let direct = value as? Data {
            data = direct
    ...
    private func captureGenericFile(_ provider: NSItemProvider) async throws -> CapturedContent? {
        ...
        return CapturedContent(
            ...
            sourceURL: nil,
            attachmentData: data,
            attachmentExtension: url.pathExtension
        )
    }
    ...
    private func recognizeText(in image: UIImage) -> String {
        ...
        let request = VNRecognizeTextRequest { request, _ in
            lines = (request.results as? [VNRecognizedTextObservation])?.compactMap {
